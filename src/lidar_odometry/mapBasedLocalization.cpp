...
public:
    ...
    ros::Subscriber subGlobalmapSubmapSurfCloud;
    ...
    std::queue<sensor_msgs::PointCloud2ConstPtr> globalmapSubmapSurfBuf;
    ...
    mapOptimization()
    {
        ...
        pubOdomAftMappedROS = nh.advertise<nav_msgs::Odometry>(PROJECT_NAME + "/lidar/mapping/odometry", 1); // 注意 imuPreintegration.cpp 中会订阅这个话题.
        ...
        pubMapWorld = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/localization/global_map", 1); //
        // fortest_publasercloudINWorld = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/mapping/laserclouinmapframe",1);
        pubLaserCloudInWorld = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/mapping/lasercloud_in_world", 1); // added
        pubOdomToMapPose = nh.advertise<geometry_msgs::PoseStamped>(PROJECT_NAME + "/localization/pose_odomTo_map", 1);
        ...
        pubRecentKeyFrames = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/lidar/mapping/current_localmap", 1);
        pubRecentKeyFrame = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/lidar/localization/cloud_registered", 1);
        pubCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/lidar/localization/cloud_registered_raw", 1);
    ...
    void allocateMemory()
    {
        ...
        cloudScanForInitialize.reset(new pcl::PointCloud<PointType>()); // addded by gc
        resetLIO();
        /************* start: added by gc*****************/
        for (int i = 0; i < 6; ++i)
        {
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void publishOdometry()
    {
        ...
        map_to_odom = tf::Transform(tf::createQuaternionFromRPY(tranformOdomToWorld[0], tranformOdomToWorld[1], tranformOdomToWorld[2]), tf::Vector3(tranformOdomToWorld[3], tranformOdomToWorld[4], tranformOdomToWorld[5]));
    ...
    void cloudGlobalLoad()
    {
            ...
            printf("gloablmap is initialized. \r\n");
    ...
    void ICPscanMatchGlobal()
    {
        ...
        *latestCloudIn += *transformPointCloud(win_surfCloudKeyFrames[latestFrameIDGlobalLocalize], &win_cloudKeyPoses6D[latestFrameIDGlobalLocalize]);
        ...
        // std::cout << " in test 0.03: roll = " << roll << " pitch = " << pitch << " yaw = " << yaw << std::endl;
...
{
    ros::init(argc, argv, "map_based_localization");

    mapOptimization MO;

    ROS_INFO("\033[1;32m----> Map-based Localization Started.\033[0m");
